...
# Таблица log_files_user_agent
    ...
    #query1 = ''' SELECT l.user_id FROM log_files_clients '''
    #df_to_fill['id_ip'] = pd.read_sql_query(query1, conn)
    df_to_fill.to_sql('temp_table', conn, if_exists='replace', index=False)
    ...
    # Таблица log_files_result
    df_to_fill = logs_df[[ 'Duration', 'Byte']].drop_duplicates()
    df_to_fill = df_to_fill.rename(
        columns={ 'Duration' : 'result_time', 'Byte': 'result_byte'})
    df_to_fill.to_sql('temp_table', conn, if_exists='replace', index=False)
...
fill_db('logs/logfile1_part.log')
